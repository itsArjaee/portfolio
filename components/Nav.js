...
const Nav = () => {
  ...
  return (
    ...
    h-max bottom-0 mt-auto xl:right-[2%] x-50 top-0 w-full xl:w-16 xl:max-w-md xl:h-screen z-10"
    >
      <div
        className="flex w-full xl:flex-col items-center justify-between xl: justify-center gap-y-10 px-4 md:px-40 
      xl:px-0 h-[80px] xl:h-max py-7 bg-white/10 backdrop-blur-sm text-3xl xl:text-xl xl:rounded-full"
      >
        {navData.map((link, index) => {
          return (
            <Link
              ...
              } relative items-center group hover:text-accent transition-all
          duration-300`}
              href={link.path}
              key={index}
            >
